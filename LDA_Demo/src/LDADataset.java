...
public class LDADataset implements Serializable {
	...
	public Dictionary localDict_movie; // local dictionary
	public Dictionary localDict_user; // local user dictionary    new
	//public Dictionary localDict_movie; // local user dictionary   new
	public static MysqlReader mr= new MysqlReader();
	public Document[] docs; // a list of documents
	public int M; // number of documents/users
	public int V; // number of words/movies

	// map from local coordinates (id) to global ones
	// null if the global dictionary is not set
	public Map<Integer, Integer> lid2gid;   //local to global
	...
	// --------------------------------------------------------------
	public LDADataset() {
		localDict_movie = new Dictionary();
		M = 0;
		V = 0;
		docs = null;
	...
	public LDADataset(MysqlReader mr) {
		localDict_user= mr.localDict_user;
		localDict_movie= mr.localDict_movie;
		this.M= mr.M;
		this.V= mr.V;
		docs = new Document[M];
	...
	public void setDoc(String str, int idx) {
		if (0 <= idx && idx < M) {
			...
			for (String word : words) {
				int _id = localDict_movie.word2id.size();   //assign _id based on temporary size
				if (localDict_movie.contains(word))
					_id = localDict_movie.getID(word);      //if not new, then update _id

				if (globalDict!= null) {
					Integer id = globalDict.getID(word);  // get the global id					
					if (id != null) {
			...
			docs[idx] = doc;  //store every user
			V = localDict_movie.word2id.size(); // number of total movies
		}
	}
	public void setDoc(String str, int idx, int i) {
		if (0 <= idx && idx < M) {
			String[] words= str.split(",");
			Vector<Integer> ids= new Vector<Integer>();		
			for (String word : words) {
				int _id = localDict_movie.getID(word);
				ids.add(_id);		
			}
			Document doc = new Document(ids, str);
			docs[idx] = doc;  //store every user
	...
	// ---------------------------------------------------------------
	/**
	 * read a dataset from MySQL, create new dictionary
	 * 
	 * @return dataset if success and null otherwise
	 */
	public static LDADataset buildDictionary() {
        mr.buildDict();
		LDADataset data = new LDADataset(mr);
		return data;
	}
	public static LDADataset readMySQL(int table) {
		try {
			LDADataset data = new LDADataset(mr);
			//System.out.println(data.M + "  " +data.V);//debug
			for(int i= 0; i< mr.M; ++i)
				data.docs[i]= new Document();  //Initializing
			//Statement stat= mr.con.createStatement();	    //创建Statement对	
			//System.out.println(mr.sqlStore[table]);
			ResultSet rs= mr.stat.executeQuery(mr.sqlStore[table]);
			//if(rs !=null) System.out.println("rs is not null !");
			while(rs.next()) {
				String user= rs.getString("deviceid");
				String line= rs.getString("title");
				int user_id= mr.localDict_user.getID(user); //read user_id from dictionary
				data.setDoc(line, user_id, 1);
				//System.out.println(user + ": " + user_id);
			}
			System.out.println("Successfully read data from table " + table);
			return data;
		} catch (Exception e) {
	...
	public static LDADataset readDataSet(MysqlReader mr) {
		try {
			mr.buildUsersDict();
			int M= mr.M;
			//int Tables= mr.sqlStore.length;
			LDADataset data = new LDADataset(M);
			//for(int i= 0; i< M; ++i)
			//	docs[i]= null;
			Statement stat= mr.con.createStatement();	    //创建Statement对			
			ResultSet rs=stat.executeQuery(mr.sqlStore[1]);
			while(rs.next()) {
				String user= rs.getString("deviceid");
				String line= rs.getString("title");
				int user_id= mr.localDict_user.getID(user); //read user_id from dictionary
				data.setDoc(line, user_id);
			}
			return data;
		} catch (Exception e) {
			System.out.println("Read Dataset Error: " + e.getMessage());
	...
	public static LDADataset readDataSet(String filename) {
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(
					new FileInputStream(filename), LDAOption.chartSet));
